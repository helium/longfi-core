...
*/
static int
blake2bp_init_leaf_param(blake2b_state * S, const blake2b_param * P) {
...
static int
blake2bp_init_leaf(blake2b_state * S, size_t outlen, size_t keylen, uint64_t offset) {
    blake2b_param P[1];
...
static int
blake2bp_init_root(blake2b_state * S, size_t outlen, size_t keylen) {
    blake2b_param P[1];
    P->digest_length = (uint8_t)outlen;
    P->key_length    = (uint8_t)keylen;
    P->fanout        = PARALLELISM_DEGREE;
...
int
blake2bp_update(blake2bp_state * S, const void * pin, size_t inlen) {
    ...
    if (left && inlen >= fill) {
        ...
        for (i = 0; i < PARALLELISM_DEGREE; ++i)
            blake2b_update(S->S[i],
                           S->buf + i * BLAKE2B_BLOCKBYTES,
                           BLAKE2B_BLOCKBYTES);
