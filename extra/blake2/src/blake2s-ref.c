...
static const uint32_t blake2s_IV[8] = {0x6A09E667UL,
                                       0xBB67AE85UL,
                                       0x3C6EF372UL,
                                       0xA54FF53AUL,
                                       0x510E527FUL,
...
static int
blake2s_is_lastblock(const blake2s_state * S) {
    return S->f[0] != 0;
}
...
static void
blake2s_compress(blake2s_state * S, const uint8_t in[BLAKE2S_BLOCKBYTES]) {
    uint32_t m[16];
    uint32_t v[16];
    size_t   i;
...
int
blake2s_update(blake2s_state * S, const void * pin, size_t inlen) {
