...
gen_payload(void * dst, size_t * dst_len_tag) {
    memset(dst, 0, CONSTANTS_MAX_PAY_LEN);
    *dst_len_tag = arc4random_uniform(CONSTANTS_MAX_PAY_LEN);
    arc4random_buf(dst, *dst_len_tag);
}

#define GEN_GEN_FLAGS(DG_TYPE)                                                 \
    void gen_##DG_TYPE##_flags(struct lfe_dg_##DG_TYPE##_flags * out) {        \
        uint32_t bits   = ru32();                                              \
...
void
gen_monolithic(struct lfe_dg_monolithic * out) {
...
void
gen_ack_flags(struct lfe_dg_ack_flags * out) {
    uint32_t bits        = ru32();
...
void
gen_frame_start(struct lfe_dg_frame_start * out) {
    out->oui = ru32();
    out->did = ru32();
    out->fp  = ru32();
    out->seq = ru32();
...
void
gen_frame_data_flags(struct lfe_dg_frame_data_flags * out) {
    out->ldpc = bit(ru32(), 0);
}

void
gen_frame_data(struct lfe_dg_frame_data * out) {
...
#define GEN_TEST(DG_TYPE)                                                      \
    TEST test_##DG_TYPE##_roundtrips() {                                       \
        ...
        struct lfe_dg_##DG_TYPE REF;                                           \
        gen_##DG_TYPE(&REF);                                                   \
                                                                               \
        /* Serialize reference. */                                             \
        uint8_t             buf_ser[1024];                                     \
        struct cursor       csr_ser = cursor_new(buf_ser, sizeof(buf_ser));    \
        enum lfe_dg_ser_res sres    = lfe_dg_##DG_TYPE##__ser(&REF, &csr_ser); \
        ASSERT_EQ(lfe_dg_ser_res_ok, sres);                                    \
                                                                               \
        /* Deserialize. */                                                     \
        struct cursor       csr_des = cursor_new(buf_ser, csr_ser.pos);        \
        struct lfe_dg_des   deserialized;                                      \
        enum lfe_dg_des_res dres = lfe_dg__des(&deserialized, &csr_des);       \
        ASSERT_EQ(lfe_dg_ser_res_ok, dres);                                    \
        ASSERT_EQ_FMT(lfe_dg_type_##DG_TYPE, deserialized.type, "%x");         \
        ...
        sres = lfe_dg_##DG_TYPE##__ser(&deserialized.DG_TYPE, &csr_reser);     \
        ASSERT_EQ(lfe_dg_ser_res_ok, sres);                                    \
                                                                               \
